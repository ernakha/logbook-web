    ...
    <div class="col-lg-12 d-flex align-items-stretch">
        <div class="card w-100 shadow-sm">
            <div class="card-header card-header-custom">
                <h5 class="mb-0 text-white">Tambah Data BKPH {{ ucfirst($daerah) }}</h5>
            </div>
            <div class="card-body p-4">
                ...
                <form action="{{ route('bkph.store', $daerah) }}" method="POST">
                    @csrf
                    <input type="hidden" name="daerah_bkph" value="{{ ucfirst($daerah) }}">

                    <div class="mb-3">
                        <label for="nama_rph" class="form-label">Nama RPH</label>
                        <input type="text" name="nama_rph" id="nama_rph"
                            class="form-control @error('nama_rph') is-invalid @enderror" required>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="pegawai_id" id="pegawai_id"
                            ...
                            <option value="{{ $item->id }}">{{ $item->user->name ?? 'Pegawai '.$item->id }}</option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="jumlah_polhuter" id="jumlah_polhuter"
                            class="form-control @error('jumlah_polhuter') is-invalid @enderror" required>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="telp_kantor" id="telp_kantor"
                            class="form-control @error('telp_kantor') is-invalid @enderror" required>
                        @error('telp_kantor') <div class="text-danger">{{ $message }}</div> @enderror
                    </div>

                    <button type="submit" class="btn btn-simpan">Simpan</button>
                    <a href="{{ route('bkph'.$daerah.'.index') }}" class="btn btn-batal">Batal</a>
                </form>
            </div>
        </div>
    </div>
